...
namespace ormpp {
    ...
    public:
        ...
        bool connect(Args &&... args) {
            auto r = sqlite3_open_v2(std::forward<Args>(args)..., &handle_,
                                     SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_NOMUTEX |
                                     SQLITE_OPEN_SHAREDCACHE, NULL);
            if (r == SQLITE_OK) {
                char *errorMsg;
                r = sqlite3_exec(handle_, "PRAGMA journal_mode=WAL;", nullptr, nullptr, &errorMsg);
                if (r == SQLITE_OK) {
                    return true;
                } else {
                    set_last_error(errorMsg);
                }
            }
...
//            }
        ...
        template<typename T, typename... Args>
        int insert(const T &t, Args &&... args, bool replace = false) {
            std::string sql = auto_key_map_.empty() ? generate_insert_sql<T>(replace) : generate_auto_insert_sql0<T>(
                    auto_key_map_, replace);

            return insert_impl(false, sql, t, std::forward<Args>(args)...);
        }

        template<typename T, typename... Args>
        int insert(const std::vector<T> &t, Args &&... args, bool replace = false) {
            std::string sql = auto_key_map_.empty() ? generate_insert_sql<T>(replace) : generate_auto_insert_sql0<T>(
                    auto_key_map_, replace);
    ...
    private:
        ...
        std::string generate_createtb_sql(Args &&... args) {
            ...
            if constexpr (SIZE > 0) {
                ...
                static_assert(!(iguana::has_type<ormpp_key, U>::value && iguana::has_type<ormpp_auto_key, U>::value),
                              "should only one key");
            }
        ...
        bool set_param_bind(T &&value, int i) {
            ...
            } else if constexpr (iguana::is_int64_v<U> || std::is_same_v<im::Date, U>) {// 解析im::Date
